    ...
    extension ABSocialManager {
        ...
        func loginToTwitter(completionHandler: SocialLoginResponseHandler? ) {
            ...
            Twitter.sharedInstance().logInWithCompletion { [weak self] (session, error) -> Void in
                ...
                if session != nil {
                    self?.twitterEmailPermission()
                }
            }
        }
        
        private func twitterEmailPermission() {
            if Twitter.sharedInstance().sessionStore.session() != nil {
//                let shareEmailViewController = TWTRShareEmailViewController()
//                self.getRootViewController()?.presentViewController(shareEmailViewController, animated: true, completion: nil)
            }
        }
    }
#endif
    ...
    extension ABSocialManager {
        ...
        private func loginToFacebookWithPermission(permission: [String], isPublish: Bool, handler: SocialLoginResponseHandler?) {
            ...
            if isPublish == false {
                fbLogin.logInWithReadPermissions(permission, fromViewController: getRootViewController(), handler: facebookHandler)
            } else {
                fbLogin.logInWithPublishPermissions(permission, fromViewController: getRootViewController(), handler: facebookHandler)
            }
        }
    }
